...
function getStoredState(config, onComplete) {
  ...
  storage.getAllKeys(function (err, allKeys) {
    if (err) {
      ...
      (0, _crashlytics.recordNonFatalError)('Persist Error', 'redux-persist/getStoredState: Error in storage.getAllKeys');
      complete(err);
    }
    ...
    keysToRestore.forEach(function (key) {
      storage.getItem(createStorageKey(key), function (err, serialized) {
        if (err) {
          ...
          (0, _crashlytics.recordNonFatalError)('Persist Error', 'redux-persist/getStoredState: Error restoring data for key:' + key);
  ...
  function rehydrate(key, serialized) {
    ...
    try {
      ...
      (0, _crashlytics.recordNonFatalError)('Persist Error', 'redux-persist/getStoredState: Error in rehydrate restoring data for key:' + key);
    }
