...
function getStoredState(config, onComplete) {
  ...
  storage.getAllKeys(function (err, allKeys) {
    if (err) {
      _reactNative.Alert.alert('redux-persist/getStoredState: Error in storage.getAllKeys');
      console.warn('redux-persist/getStoredState: Error in storage.getAllKeys');
      (0, _crashlytics.recordNonFatalError)('Persist Error', 'redux-persist/getStoredState: Error in' + ' storage.getAllKeys ' + err.toString());
      complete(err);
    }
    ...
    (0, _crashlytics.recordNonFatalError)('Persist Error', 'redux-persist/allkeys: ' + allKeys + ' ' + keysToRestore);
    ...
    keysToRestore.forEach(function (key) {
      storage.getItem(createStorageKey(key), function (err, serialized) {
        if (err) {
          _reactNative.Alert.alert('redux-persist/getStoredState: Error restoring data for key:' + key);
          console.warn('redux-persist/getStoredState: Error restoring data for key:', key, err);
          (0, _crashlytics.recordNonFatalError)('Persist Error', 'redux-persist/getStoredState: Error restoring' + ' data for key:' + key + ' ' + err.toString());
  ...
  function rehydrate(key, serialized) {
    ...
    try {
      ...
      _reactNative.Alert.alert('redux-persist/getStoredState: Error in rehydrate restoring data for key:' + key);
      (0, _crashlytics.recordNonFatalError)('Persist Error', 'redux-persist/getStoredState: Error in rehydrate' + ' restoring data for key:' + key + ' ' + (err || '').toString());
    }
