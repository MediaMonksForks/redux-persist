  ...
  store.subscribe(function () {
    ...
    if (timeIterator === null) {
      timeIterator = setInterval(function () {
        ...
        if ((0, _lodash.isEmpty)(endState)) {
          _reactNative.Alert.alert('redux-persist/subscribe: Saving an empty value for' + key);
          (0, _crashlytics.recordNonFatalError)('Persist Error', 'redux-persist/subscribe: Saving an empty value for' + key);
        }
    ...
    if (options.serial) {
      stateIterator(incoming, function (subState, key) {
        try {
          ...
          _reactNative.Alert.alert('redux-persist/adhocRehydrate: Error rehydrating data for key "' + key + '"');
          (0, _crashlytics.recordNonFatalError)('Persist Error', 'redux-persist/adhocRehydrate: Error rehydrating data for key' + key + ' ' + (err || '').toString());
        }
      });
    } else state = incoming;
...
function warnIfSetError(key) {
  return function setError(err) {
    if (err) {
      ...
      _reactNative.Alert.alert('redux-persist/warnIfSetError: Error storing data for key:' + key);
      (0, _crashlytics.recordNonFatalError)('Persist Error', 'redux-persist/warnIfSetError: Error storing data' + ' for key:' + key + ' ' + (err || '').toString());
    }
  };
}
