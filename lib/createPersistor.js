...
function createPersistor(deviceID, store, config) {
  // defaults
    ...
    if (options.serial) {
      stateIterator(incoming, function (subState, key) {
        try {
          ...
          _reactNative.Alert.alert('redux-persist/adhocRehydrate: Error rehydrating data for key "' + key + '"  ' + (err || '').toString());
  ...
  return {
    ...
    purge: function purge(keys) {
      return (0, _purgeStoredState2.default)(deviceID, { storage: storage, keyPrefix: keyPrefix }, keys);
    }
  };
}
...
function warnIfSetError(key, deviceID) {
  return function setError(err) {
    if (err) {
      ...
      _reactNative.Alert.alert('redux-persist/warnIfSetError: Error storing data for key:' + key + (err || '').toString());
      (0, _crashlytics.recordNonFatalError)('Persist Error', deviceID + ' redux-persist/warnIfSetError: Error' + ' storing data' + ' for key:' + key + ' ' + (err || '').toString());
    }
  };
}
